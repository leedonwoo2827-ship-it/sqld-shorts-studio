...
쇼츠는 **한 문제 = 한 파일 = 한 영상**. SQLD 실전 샘플: `samples/sqld_shorts/sqld_01.json … sqld_10.json`.
...
### 2) 실행 — `run.bat` 더블클릭
브라우저가 `http://localhost:8830` 로 자동으로 열립니다.
...
2. **compy-ui `run.bat`** → 브라우저 `http://localhost:8830`
3. 웹에서 대본 → 이미지 → 음성 → MP4
